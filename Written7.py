...
#Add school district location checks
import geopandas as gpd
district_areas = gpd.read_file(
    r"C:\Users\madig\OneDrive\Desktop\IDX Exchange\DistrictAreas2526_-284845464123469011.geojson"
)
unified_districts = district_areas.loc[
    district_areas["DistrictType"] == "Unified"
].copy()
...
# flag missing coordinate records
sold_with_rates['missing_coordinate_flag'] = (
    sold_with_rates['Longitude'].isna() |
    sold_with_rates['Latitude'].isna() |
    (sold_with_rates['Longitude'] == 0) |
    (sold_with_rates['Latitude'] == 0)
)

# flag invalid California coordinates (Longitude should be negative)
sold_with_rates['invalid_longitude_flag'] = sold_with_rates['Longitude'] > 0

# flag implausible or out-of-state coordinates for California
sold_with_rates['invalid_coordinate_flag'] = (
    sold_with_rates['Longitude'].isna() |
    sold_with_rates['Latitude'].isna() |
    (sold_with_rates['Longitude'] == 0) |
    (sold_with_rates['Latitude'] == 0) |
    (sold_with_rates['Longitude'] > 0) |
    (sold_with_rates['Longitude'] < -125) |
    (sold_with_rates['Latitude'] < 32) |
    (sold_with_rates['Latitude'] > 42)
)

# drop rows missing coordinates, sentinel zeros, or implausible values
sold_with_rates = sold_with_rates.dropna(subset=['Longitude', 'Latitude'])
sold_with_rates = sold_with_rates[~sold_with_rates['invalid_coordinate_flag']]

# convert to GeoDataFrame
gdf = gpd.GeoDataFrame(
    sold_with_rates,
    geometry=gpd.points_from_xy(sold_with_rates['Longitude'], sold_with_rates['Latitude']),
    crs="EPSG:4326"
)
# align CRSs
if gdf.crs != unified_districts.crs:
    unified_districts = unified_districts.to_crs(gdf.crs)

# spatial join: property point inside district polygon
properties_with_district = gpd.sjoin(
    gdf,
    unified_districts,
    how="left",
    predicate="within"
...
# keep the district name in the joined GeoDataFrame
joined['DistrictName'] = joined['DistrictName']

# in the original DataFrame
sold_with_rates = sold_with_rates.loc[joined.index].copy()
sold_with_rates['DistrictName'] = joined['DistrictName'].values

#New Values (Key Metrics) for Sold
sold_with_rates['close_price_ratio'] = sold_with_rates['ClosePrice'] / sold_with_rates.get('OriginalListPrice', sold_with_rates.get('ListPrice'))
sold_with_rates['close_to_original_list_ratio'] = sold_with_rates['ClosePrice'] / sold_with_rates['OriginalListPrice'] if 'OriginalListPrice' in sold_with_rates.columns else pd.NA
sold_with_rates['price_per_sqft'] = sold_with_rates['ClosePrice'] / sold_with_rates['LivingArea']
...
sold_with_rates['listing_to_contract_days'] = (
    (sold_with_rates['PurchaseContractDate'] - sold_with_rates['ListingContractDate']).dt.days
)
sold_with_rates['contract_to_close_days'] = (
    (sold_with_rates['CloseDate'] - sold_with_rates['PurchaseContractDate']).dt.days
)

# print a sample of about 10 rows
sample = sold_with_rates.loc[:, [
    'CloseDate',
    'ListingContractDate',
...
full_sold_summary = (
    sold_with_rates.groupby('PropertyType')[summary_cols]
    .agg(['count', 'mean', 'median', 'std', 'min', 'max'])
)

print(full_sold_summary)

#Print Summary by Property Subtype
pd.options.display.float_format = '{:.2f}'.format
...
full_sold_summary = (
    sold_with_rates.groupby('PropertySubType')[summary_cols]
    .agg(['count', 'mean', 'median', 'std', 'min', 'max'])
)

print(full_sold_summary)

#Print Summary by County or Parish
pd.options.display.float_format = '{:.2f}'.format
...
full_sold_summary = (
    sold_with_rates.groupby('CountyOrParish')[summary_cols]
    .agg(['count', 'mean', 'median', 'std', 'min', 'max'])
)

print(full_sold_summary)

#Print Summary by MLS Area Major
pd.options.display.float_format = '{:.2f}'.format
...
full_sold_summary = (
    sold_with_rates.groupby('MLSAreaMajor')[summary_cols]
    .agg(['count', 'mean', 'median', 'std', 'min', 'max'])
)

print(full_sold_summary)

#Print Summary by List Office Name
pd.options.display.float_format = '{:.2f}'.format
...
full_sold_summary = (
    sold_with_rates.groupby('ListOfficeName')[summary_cols]
    .agg(['count', 'mean', 'median', 'std', 'min', 'max'])
)

print(full_sold_summary)

#Print Summary by Buyer Office Name
pd.options.display.float_format = '{:.2f}'.format
...
full_sold_summary = (
    sold_with_rates.groupby('BuyerOfficeName')[summary_cols]
    .agg(['count', 'mean', 'median', 'std', 'min', 'max'])
)

print(full_sold_summary)

#Flagging Outliers for Sold
CQ1 = sold_with_rates['ClosePrice'].quantile(0.25)
CQ3 = sold_with_rates['ClosePrice'].quantile(0.75)
CIQR = CQ3 - CQ1
Clower = CQ1 - 1.5 * CIQR
Cupper = CQ3 + 1.5 * CIQR
sold_with_rates['outlier_flag'] = (
    sold_with_rates['ClosePrice'] < Clower
) | (
    sold_with_rates['ClosePrice'] > Cupper
)

print(f"Outlier flag count for ClosePrice: {sold_with_rates['outlier_flag'].sum()}")
#Outlier flag count for ClosePrice: 31,998
LQ1 = sold_with_rates['LivingArea'].quantile(0.25)
LQ3 = sold_with_rates['LivingArea'].quantile(0.75)
LIQR = LQ3 - LQ1
Llower = LQ1 - 1.5 * LIQR
Lupper = LQ3 + 1.5 * LIQR
sold_with_rates['outlier_flag'] = (
    sold_with_rates['LivingArea'] < Llower
) | (
    sold_with_rates['LivingArea'] > Lupper
)

print(f"Outlier flag count for LivingArea: {sold_with_rates['outlier_flag'].sum()}")
#Outlier flag count for LivingArea: 18,880
DQ1 = sold_with_rates['DaysOnMarket'].quantile(0.25)
DQ3 = sold_with_rates['DaysOnMarket'].quantile(0.75)
DIQR = DQ3 - DQ1
Dlower = DQ1 - 1.5 * DIQR
Dupper = DQ3 + 1.5 * DIQR
sold_with_rates['outlier_flag'] = (
    sold_with_rates['DaysOnMarket'] < Dlower
) | (
    sold_with_rates['DaysOnMarket'] > Dupper
)

print(f"Outlier flag count for DaysOnMarket: {sold_with_rates['outlier_flag'].sum()}")
#Outlier flag count for DaysOnMarket: 32,186
sold_with_rates.to_csv(r"C:\Users\madig\OneDrive\Desktop\IDX Exchange\Week 1\ResidentialSold.csv", index=False)
#See datasize and median values before removing outliers
print(len(sold_with_rates))
#Data size=431157 observations
print(sold_with_rates['ClosePrice'].median())
#Close Price Median=819,000
print(sold_with_rates['LivingArea'].median())
#Living Area Median=1,649
print(sold_with_rates['DaysOnMarket'].median())
#Days on Market Median=19
#Create a data set with Outliers Removed for Sold
sold_without_outliers = sold_with_rates[(sold_with_rates['ClosePrice'] >= Clower) & (sold_with_rates['ClosePrice'] <= Cupper)]
sold_without_outliers = sold_with_rates[(sold_with_rates['LivingArea'] >= Llower) & (sold_with_rates['LivingArea'] <= Lupper)]
sold_without_outliers = sold_with_rates[(sold_with_rates['DaysOnMarket'] >= Dlower) & (sold_with_rates['DaysOnMarket'] <= Dupper)]
sold_without_outliers.to_csv(r"C:\Users\madig\OneDrive\Desktop\IDX Exchange\Week 1\ResidentialSoldWithoutOutliers.csv", index=False)
#See datasize and median values after removing outliers
print(len(sold_without_outliers))
#Data size=398,971 observations
print(sold_without_outliers['ClosePrice'].median())
#Close Price Median=825,000
print(sold_without_outliers['LivingArea'].median())
#Living Area Median=1,640
print(sold_without_outliers['DaysOnMarket'].median())
#Days on Market Median=16
...
#Add school district location checks
# ensure numeric coordinates
listings_with_rates['Longitude'] = pd.to_numeric(listings_with_rates['Longitude'], errors='coerce')
listings_with_rates['Latitude'] = pd.to_numeric(listings_with_rates['Latitude'], errors='coerce')

# flag missing coordinate records
listings_with_rates['missing_coordinate_flag'] = (
    listings_with_rates['Longitude'].isna() |
    listings_with_rates['Latitude'].isna() |
    (listings_with_rates['Longitude'] == 0) |
    (listings_with_rates['Latitude'] == 0)
)

# flag invalid California coordinates (Longitude should be negative)
listings_with_rates['invalid_longitude_flag'] = listings_with_rates['Longitude'] > 0

# flag implausible or out-of-state California coordinates
listings_with_rates['invalid_coordinate_flag'] = (
    listings_with_rates['Longitude'].isna() |
    listings_with_rates['Latitude'].isna() |
    (listings_with_rates['Longitude'] == 0) |
    (listings_with_rates['Latitude'] == 0) |
    (listings_with_rates['Longitude'] > 0) |
    (listings_with_rates['Longitude'] < -125) |
    (listings_with_rates['Latitude'] < 32) |
    (listings_with_rates['Latitude'] > 42)
)

# drop rows missing coordinates, sentinel zeros, or implausible values
listings_with_rates = listings_with_rates.dropna(subset=['Longitude', 'Latitude'])
listings_with_rates = listings_with_rates[~listings_with_rates['invalid_coordinate_flag']]

# convert to GeoDataFrame
gdf = gpd.GeoDataFrame(
    listings_with_rates,
    geometry=gpd.points_from_xy(listings_with_rates['Longitude'], listings_with_rates['Latitude']),
    crs="EPSG:4326"
)
# align CRSs
if gdf.crs != unified_districts.crs:
...
# in the original DataFrame
listings_with_rates = listings_with_rates.loc[joined.index].copy()
listings_with_rates['DistrictName'] = joined['DistrictName'].values

#New Values (Key Metrics) for Listings
listings_with_rates['price_ratio'] = listings_with_rates['ClosePrice'] / listings_with_rates.get('OriginalListPrice', listings_with_rates.get('ListPrice'))
listings_with_rates['close_to_original_list_ratio'] = listings_with_rates['ClosePrice'] / listings_with_rates['OriginalListPrice'] if 'OriginalListPrice' in listings_with_rates.columns else pd.NA
listings_with_rates['price_per_sqft'] = listings_with_rates['ClosePrice'] / listings_with_rates['LivingArea']
listings_with_rates['Year'] = listings_with_rates['CloseDate'].dt.year
listings_with_rates['Month'] = listings_with_rates['CloseDate'].dt.month
listings_with_rates['YrMo'] = listings_with_rates['CloseDate'].dt.to_period('M').astype(str)
listings_with_rates['listing_to_contract_days'] = (
    (listings_with_rates['PurchaseContractDate'] - listings_with_rates['ListingContractDate']).dt.days
)
listings_with_rates['contract_to_close_days'] = (
    (listings_with_rates['CloseDate'] - listings_with_rates['PurchaseContractDate']).dt.days
)

# print a sample of about 10 rows
sample = listings_with_rates.loc[:, [
    'CloseDate',
    'ListingContractDate',
    'PurchaseContractDate',
    'ClosePrice',
...
full_listing_summary = (
    sold_with_rates.groupby('PropertyType')[summary_cols]
    .agg(['count', 'mean', 'median', 'std', 'min', 'max'])
)

print(full_listing_summary)
...
full_listing_summary = (
    listings_with_rates.groupby('PropertySubType')[summary_cols]
    .agg(['count', 'mean', 'median', 'std', 'min', 'max'])
)
...
full_listing_summary = (
    listings_with_rates.groupby('CountyOrParish')[summary_cols]
    .agg(['count', 'mean', 'median', 'std', 'min', 'max'])
)
...
full_listing_summary = (
    listings_with_rates.groupby('MLSAreaMajor')[summary_cols]
    .agg(['count', 'mean', 'median', 'std', 'min', 'max'])
)
...
full_listing_summary = (
    listings_with_rates.groupby('ListOfficeName')[summary_cols]
    .agg(['count', 'mean', 'median', 'std', 'min', 'max'])
)
...
full_listing_summary = (
    listings_with_rates.groupby('BuyerOfficeName')[summary_cols]
    .agg(['count', 'mean', 'median', 'std', 'min', 'max'])
)

print(full_listing_summary)

#Flagging Outliers for Listing
CQ1 = listings_with_rates['ClosePrice'].quantile(0.25)
CQ3 = listings_with_rates['ClosePrice'].quantile(0.75)
CIQR = CQ3 - CQ1
Clower = CQ1 - 1.5 * CIQR
Cupper = CQ3 + 1.5 * CIQR
listings_with_rates['outlier_flag'] = (
    listings_with_rates['ClosePrice'] < Clower
) | (
    listings_with_rates['ClosePrice'] > Cupper
)

print(f"Outlier flag count for ClosePrice: {listings_with_rates['outlier_flag'].sum()}")
#Outlier flag count for ClosePrice: 9,788
LQ1 = listings_with_rates['LivingArea'].quantile(0.25)
LQ3 = listings_with_rates['LivingArea'].quantile(0.75)
LIQR = LQ3 - LQ1
Llower = LQ1 - 1.5 * LIQR
Lupper = LQ3 + 1.5 * LIQR
listings_with_rates['outlier_flag'] = (
    listings_with_rates['LivingArea'] < Llower
) | (
    listings_with_rates['LivingArea'] > Lupper
)

print(f"Outlier flag count for LivingArea: {listings_with_rates['outlier_flag'].sum()}")
#Outlier flag count for LivingArea: 5,660
DQ1 = listings_with_rates['DaysOnMarket'].quantile(0.25)
DQ3 = listings_with_rates['DaysOnMarket'].quantile(0.75)
DIQR = DQ3 - DQ1
Dlower = DQ1 - 1.5 * DIQR
Dupper = DQ3 + 1.5 * DIQR
listings_with_rates['outlier_flag'] = (
    listings_with_rates['DaysOnMarket'] < Dlower
) | (
    listings_with_rates['DaysOnMarket'] > Dupper
)

print(f"Outlier flag count for DaysOnMarket: {listings_with_rates['outlier_flag'].sum()}")
#Outlier flag count for DaysOnMarket: 10,162
listings_with_rates.to_csv(r"C:\Users\madig\OneDrive\Desktop\IDX Exchange\Week 1\ResidentialListing.csv", index=False)
#See datasize and median values before removing outliers
print(len(listings_with_rates))
#Data size=130,265 observations
print(listings_with_rates['ClosePrice'].median())
#Close Price Median=840,000
print(listings_with_rates['LivingArea'].median())
#Living Area Median=1,629
print(listings_with_rates['DaysOnMarket'].median())
#Days on Market Median=12
#Create a data set with Outliers Removed for Sold
listings_without_outliers = listings_with_rates[(listings_with_rates['ClosePrice'] >= Clower) & (listings_with_rates['ClosePrice'] <= Cupper)]
listings_without_outliers = listings_with_rates[(listings_with_rates['LivingArea'] >= Llower) & (listings_with_rates['LivingArea'] <= Lupper)]
listings_without_outliers = listings_with_rates[(listings_with_rates['DaysOnMarket'] >= Dlower) & (listings_with_rates['DaysOnMarket'] <= Dupper)]
listings_without_outliers.to_csv(r"C:\Users\madig\OneDrive\Desktop\IDX Exchange\Week 1\ResidentialListingWithoutOutliers.csv", index=False)
#See datasize and median values after removing outliers
print(len(listings_without_outliers))
#Data size=120,103 observations
print(listings_without_outliers['ClosePrice'].median())
#Close Price Median=849,000
print(listings_without_outliers['LivingArea'].median())
#Living Area Median=1,622
print(listings_without_outliers['DaysOnMarket'].median())
#Days on Market Median=11
